...
async function testServers() {
	...
	for (const serverItem of registry.servers) {
		...
		const args = pkg.registryType === "npm" ? ["-y", pkg.identifier] : [];
		...
		if (pkg.packageArguments) {
			pkg.packageArguments.forEach((arg) => {
				...
				if (val.includes("${browser_url}")) {
					// biome-ignore lint/suspicious/noTemplateCurlyInString: placeholder syntax
					val = val.replace("${browser_url}", "http://localhost:9222");
				}
				args.push(val);
			});
		}
		...
		const success = await new Promise((resolve) => {
			const timeout = setTimeout(() => {
				console.log(
					`✅ Server "${id}" started successfully (no immediate crash).`,
				);
				child.kill();
				resolve(true);
			}, 3000);
